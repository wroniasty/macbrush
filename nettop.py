...
class IfStatDevice(Agent):

    def __init__(self, ip, interval=3):
        super(IfStatDevice, self).__init__(ip)
        self.target_ip = ip
        ...
        if results:
            for index, iff in results['interfaces'].items():
                ...
                for stat_name in if_data['history']:
                    self.push_stat_data_point(if_data, stat_name, ts, iff.get(stat_name, 0))
    ...
    def push_stat_data_point(self, if_data, stat_name, timestamp, value):
        ...
        try:
            if_data['history'][stat_name].append((timestamp, value))
            H = if_data['history'][stat_name]
            if len(H) >= 2:
                rate = if_data['rate'][stat_name] = calculate_rate(H[-1], H[-2])
                if_data['bitrate'][stat_name] = rate * 8
                return rate
        except KeyError:
            pass
...
class IfStatCursesViewer(Screen):
    ...
    def update_display(self):
        ...
        map (lambda p: p.clear(), self.panels.values())

        col_off = 5
        row_off = 2
        data_slice = slice(0,self.rows-5)

        if 129 in self.available_types:
            win = self.panels[129]
            for idx, iff in enumerate(ifOfType(data,129)[data_slice]):
                row = idx + row_off
        ...
        if 128 in self.available_types:
            win = self.panels[128]
            for idx, iff in enumerate(ifOfType(data,128)[data_slice]):
                row = idx + row_off
                win.addstr(row, col_off + 0, shortenText(iff["Descr"],16), curses.color_pair(1) )
                if iff["AdminStatus"] == 1:
                    win.addstr(row, col_off + 18, "up", curses.color_pair(1) )
        ...
        if 6 in self.available_types:
            win = self.panels[6]
            for idx, iff in enumerate(ifOfType(data,6)[data_slice]):
                row = idx + row_off
                win.addstr(row, col_off + 0, shortenText(iff["Descr"],16), curses.color_pair(1) )
                if iff["AdminStatus"] == 1:
                    win.addstr(row, col_off + 18, "up", curses.color_pair(1) )
...
def main(argv):
    ...
    topper = IfStatDevice("10.1.0.5")
    topper.run()
    screen = IfStatCursesViewer(topper)
